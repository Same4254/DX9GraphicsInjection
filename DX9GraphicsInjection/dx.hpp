...
typedef HRESULT(APIENTRY* BeginScene)(LPDIRECT3DDEVICE9 pDevice);
typedef HRESULT(APIENTRY* EndScene)(LPDIRECT3DDEVICE9 pDevice);
typedef HRESULT(APIENTRY* SetTransform)(LPDIRECT3DDEVICE9 o_pDevice, D3DTRANSFORMSTATETYPE State, const D3DMATRIX* pMatrix);

// Length of the vtable in bytes. 119 entries at 8 bytes each (64 bit pointer)
#define DEVICE_VTABLE_SIZE 952
...
public:
	static bool GetD3D9DeviceVTable(void** vTable, size_t size);
};

//Copied from d3d9.h: list of functions in the vTable in the correct order
